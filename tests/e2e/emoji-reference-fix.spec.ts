...
for(const emoji of ["🙏","🙏🙂","👩🏽‍💻❤️🇺🇳1️⃣"])test(`fixed route maps selected Unicode evidence and five reviewed contexts for ${emoji}`,async({page})=>{
  ...
  expect(payload.context.map((entry:{label:string})=>entry.label)).toEqual(["c0","c1","c2","c3","c4"]);
  ...
  const explanation:Explanation=result.body.result.explanation;
  expect(explanation.contextualInterpretations.map(entry=>entry.text)).toEqual(["This may be a thankful response.","It could also soften a request."]);
  const panel=page.getByRole("heading",{name:"Possible meaning",exact:true}).locator("..");
  const brief=panel.getByText("Possible meaning here: "+explanation.contextualInterpretations[0].text,{exact:true});
  await expect(brief).toBeVisible();
  const toggle=panel.locator("summary").filter({hasText:/^Details$/}),details=toggle.locator("..");
  await expect(toggle).toHaveText("Details");await expect(details).not.toHaveAttribute("open");
  const statements=[...explanation.observations.map(entry=>entry.text),...explanation.commonUsage,
    ...explanation.contextualInterpretations.map(entry=>entry.text),...explanation.uncertainties,...explanation.safeResponseGuidance];
  for(const statement of statements)await expect(details.getByText(statement,{exact:true})).toBeHidden();
  expect(requests).toHaveLength(1);
  await toggle.click();await expect(details).toHaveAttribute("open","");
  await expect(details.getByRole("heading",{name:"Common usage",exact:true})).toBeVisible();
  for(const statement of statements)await expect(details.getByText(statement,{exact:true})).toBeVisible();
  for(const id of [ids[2],ids[4]])await expect(details.getByText("Context references: "+id,{exact:true})).toBeVisible();
  expect(requests).toHaveLength(1);
  await toggle.click();await expect(details).not.toHaveAttribute("open");
  for(const statement of statements)await expect(details.getByText(statement,{exact:true})).toBeHidden();
  await expect(brief).toBeVisible();expect(requests).toHaveLength(1);
});
for(const kind of ["named-background","foreign-context","fake-frame"] as const)test(`fixed route still rejects ${kind} without sanitizing or retrying`,async({page})=>{
  ...
  expect(result.status).toBe(400);expect(result.body.code).toBe("model-output-invalid-references");
  await expect(page.getByRole("heading",{name:"Possible meaning",exact:true})).toHaveCount(0);expect(requests).toHaveLength(1);
});
