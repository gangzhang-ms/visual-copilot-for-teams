...
export function SpeakerProfileEditor({room,speaker,role,language,busy,onDirty,onSave}:{room:LocalState;speaker:string;role:"sender"|"outgoing";language:Language;busy:boolean;onDirty:(dirty:boolean)=>void;onSave:(profile:SpeakerProfile|null)=>Promise<boolean>}){
  ...
  const previousSpeaker=useRef(speaker);
  useEffect(()=>{
    const changed=previousSpeaker.current!==speaker;previousSpeaker.current=speaker;
    if(!changed&&dirty)return;
    setDraft(stored??emptySpeakerProfile());setDirty(false);onDirty(false);
  },[speaker,serialized]);
  async function save(profile:SpeakerProfile|null){
    if(await onSave(profile)){setDraft(profile??emptySpeakerProfile());setDirty(false);onDirty(false);}
  }
  ...
  return <details className="speaker-profile"><summary>{role==="sender"?t("Selected sender profile","所选消息发言者偏好"):t("Outgoing speaker profile","当前发言者偏好")}: <strong>{speaker||"—"}</strong> · {stored?t("self-reported","自愿填写"):t("unknown","未知")}</summary>
    ...
    <button disabled={busy||!dirty||!speaker.trim()} onClick={()=>void save(draft)}>{t("Save speaker profile","保存发言者偏好")}</button>
    <button disabled={busy||!stored} onClick={()=>void save(null)}>{t("Remove speaker profile","移除发言者偏好")}</button>
